...
function cardTexture(atlas:HTMLImageElement,index:number){
 ...
 x.fillStyle='#d9dfe5';x.fillRect(0,0,512,896);
 x.strokeStyle='#acb8c6';x.lineWidth=2;x.strokeRect(14,14,484,868);
 x.fillStyle='#677684';x.font='22px sans-serif';x.fillText(`VOL.06     /     ${String(index+1).padStart(2,'0')}`,40,64);
 const sx=(index%4)*512,sy=Math.floor(index/4)*288;x.drawImage(atlas,sx,sy,512,288,24,95,464,261);
 x.fillStyle='#263743';x.font='32px serif';
 ...
 x.fillStyle='#687984';x.font='21px sans-serif';x.fillText('王丛 · 地瓜机器人',40,600);x.fillText(`${formatTime(chapter.start)} — ${formatTime(chapter.end)}`,40,642);
 x.strokeStyle='#a9b7c1';x.beginPath();x.moveTo(40,701);x.lineTo(472,701);x.stroke();
 x.fillStyle='#304c60';x.beginPath();x.arc(256,783,28,0,Math.PI*2);x.fill();x.fillStyle='#ecf3f6';x.beginPath();x.moveTo(250,771);x.lineTo(250,795);x.lineTo(268,783);x.fill();
...
export async function createVideoSlices(video:HTMLVideoElement,center:number[]){
 ...
 const radius=4.6;
 const backingMaterial=new THREE.MeshStandardMaterial({color:0x8896a4,metalness:.3,roughness:.4});
