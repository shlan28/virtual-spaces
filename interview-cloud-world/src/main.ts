...
const video=document.createElement('video');video.src=interview.src;video.preload='metadata';video.playsInline=true;video.controls=true;video.poster='/media/chapter-atlas.jpg';
...
const ui=createInterface({video,chapters,
 ...
 onSliceStep:direction=>{slices?.step(direction);if(slices)ui.setSliceControls(true,chapters[slices.current].title);},onOpenSlice:()=>{if(slices)openChapter(slices.current);},
 onChapter:i=>openChapter(i),onPlay:()=>{void video.play().catch(()=>ui.showError('请点击播放器中的播放按钮开始观看。'));},
 onClose:closePanel,onQuality:()=>{lowQuality=!lowQuality;world?.setLowQuality(lowQuality);}
});
function closePanel(){video.pause();ui.closePanel();if(panelOpen)navigation?.restore();panelOpen=false;}
...
function up(event:PointerEvent){
 ...
 else if(data.pending){if(!panelOpen)navigation?.save();panelOpen=true;ui.showInsight('下一场对话，留在云端','目前接入的是第六期真实访谈。这里预留给未来访谈，不代表已有节目或嘉宾。',()=>{closePanel();navigation?.navigate('interview');});}
}
...
async function start(){
 try{
  ...
  navigation=createNavigation(world.camera,renderer.domElement,world.layout,world.colliders,id=>{const d=destinations[id];ui.setLocation(d.name,d.description);ui.setActiveDestination(id);ui.setSliceControls(id==='interview',chapters[slices?.current??2].title);});
