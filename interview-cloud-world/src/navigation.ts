...
export function createNavigation(camera:THREE.PerspectiveCamera,canvas:HTMLCanvasElement,layout:WorldLayout,onLocation:(id:string)=>void){
 ...
 let journey=0,lateral=0,id='entry',last=0,anim:null|{from:THREE.Vector3;to:THREE.Vector3;targetFrom:THREE.Vector3;targetTo:THREE.Vector3;t:number}=null;
 ...
 function navigate(next:string){const d=destinations[next];if(!d)return;id=next;lateral=0;anim={from:camera.position.clone(),to:new THREE.Vector3().fromArray(d.position),targetFrom:controls.target.clone(),targetTo:new THREE.Vector3().fromArray(d.target),t:0};onLocation(next);journey=next==='summit'?1:next==='arch'?.52:0;}
 const isInput=(e:KeyboardEvent)=>e.target instanceof HTMLElement&&!!e.target.closest('input,textarea,button,video,[role="dialog"]');
 function down(e:KeyboardEvent){if(isInput(e))return;if(['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown'].includes(e.code)){keySet.add(e.code);e.preventDefault();}}
 function up(e:KeyboardEvent){keySet.delete(e.code);}
 ...
 return {controls,navigate,
  ...
  update(time:number){const dt=Math.min(time-last,.05);last=time;
   ...
   const {forward:movement,strafe}=movementAxes(keySet);
   if(movement||strafe){anim=null;const scale=diagonalScale(movement,strafe);journey=THREE.MathUtils.clamp(journey+movement*scale*dt*.045,0,1);lateral=nextLateral(lateral,strafe,dt,2.4*scale,2.4);const p=curve.getPoint(journey),q=curve.getPoint(Math.min(1,journey+.07));routeRight.set(-(q.z-p.z),0,q.x-p.x).normalize();p.addScaledVector(routeRight,lateral);q.addScaledVector(routeRight,lateral);p.y=Math.max(p.y,surfaceHeight(layout,p.x,p.z)+.3)+2.0;q.y=Math.max(q.y,surfaceHeight(layout,q.x,q.z)+.3)+1.8;camera.position.lerp(p,.15);controls.target.lerp(q,.12);}
   controls.update();
  },
