...
export function createInterface(options: InterfaceOptions) {
  ...
  const locationEyebrow = element('div', 'location-eyebrow', '01 / 峡谷入口');
  const locationTitle = element('h1', 'location-title', '沿着光，走进对话。');
  ...
  hint.append(element('span', 'desktop-navigation-hint', '拖动观察 · WASD 自由移动 · 点击展品'), element('span', 'touch-navigation-hint', '拖动观察 · 点击展品探索'));
  ...
  const loadingTitle = element('h2', 'loading-title', '一场对话，一处回响。');
